...
class UserAccount(models.Model):
    ...
    last_name = models.CharField(max_length=255) # Set to null if no value
    mid_initial = models.CharField(max_length=255)
    account_type = models.CharField(
        max_length=2,
        choices=AccountType.choices,
        default=AccountType.INTERN) # Set Intern if no response
...
class TaskAssignment(models.Model):
    task_assignment_id = models.IntegerField(primary_key=True, verbose_name='Task Assignment ID')
...
class Department(models.Model):
    department_id = models.IntegerField(primary_key=True, verbose_name='Department ID')
...
class DepartmentSupervisor(models.Model):
    sup_id = models.IntegerField(primary_key=True, verbose_name='User ID')
    user_id = models.ForeignKey('UserAccount', on_delete=models.CASCADE, verbose_name='User ID')
    department_id = models.ForeignKey('Department', on_delete=models.CASCADE, verbose_name='Department ID')

    def __str__(self):
        return f"Department Supervisor - User ID: {self.user_id}, Department ID: {self.department_id}"
...
class Workload(models.Model):
    ...
    intern_id = models.IntegerField()  # Assuming this is the foreign key for the user ID
